...
const getChunks = (essay: PGEssay) => {
  ...
  if (encode(content).length > CHUNK_SIZE) {
    ...
    for (let i = 0; i < split.length; i++) {
      ...
      const ChunkTokenLength = encode(chunkText).length;

      if (ChunkTokenLength + sentenceTokenLength > CHUNK_SIZE) {
  ...
  const essayChunks = essayTextChunks.map((chunkText, i) => {
    const chunk: PGChunk = {
      essay_title: title,
      essay_url: url,
      essay_date: date,
...
(async () => {
  ...
  for (let i = 0; i < links.length; i++) {
    ...
    if (i === 7) {
      const essay = await getEssays(link.url, link.title);
